...
pub trait KlubuRepository: Send + Sync {
    ...
    fn get_contacts(
        ...
        query: Option<String>,
        archived: bool,
    ) -> impl std::future::Future<Output = Result<Page<Contact>, ServerFnError>> + Send;
    fn get_contact(
        &self,
        id: i64,
    ) -> impl std::future::Future<Output = Result<Contact, ServerFnError>> + Send;
    fn save_contact(
        &self,
        contact: Contact,
    ) -> impl std::future::Future<Output = Result<Contact, ServerFnError>> + Send;
    fn archive_contact(
        &self,
        id: i64,
    ) -> impl std::future::Future<Output = Result<(), ServerFnError>> + Send;
    fn restore_contact(
        &self,
        id: i64,
    ) -> impl std::future::Future<Output = Result<(), ServerFnError>> + Send;
    ...
    fn get_dashboard_stats(
        &self,
    ) -> impl std::future::Future<Output = Result<DashboardStats, ServerFnError>> + Send;
    ...
    fn get_document_meta(
        &self,
        doc_id: i32,
    ) -> impl std::future::Future<Output = Result<Option<(String, String, String)>, ServerFnError>> + Send;
    fn get_latest_document_version(
        &self,
        doc_id: i32,
    ) -> impl std::future::Future<Output = Result<Option<(i32, i32)>, ServerFnError>> + Send;

    // Exports
    fn update_invoice_document(
        &self,
        invoice_id: i64,
        doc_id: i32,
    ) -> impl std::future::Future<Output = Result<(), ServerFnError>> + Send;
    fn update_offer_document(
        &self,
        offer_id: i64,
        doc_id: i32,
        revision: i32,
    ) -> impl std::future::Future<Output = Result<(), ServerFnError>> + Send;
    fn update_receipt_document(
        &self,
        receipt_id: i64,
        doc_id: i32,
    ) -> impl std::future::Future<Output = Result<(), ServerFnError>> + Send;
    ...
    ) -> impl std::future::Future<Output = Result<Page<InvoiceListItem>, ServerFnError>> + Send;
    fn get_invoice(
        &self,
        id: i64,
    ...
    fn get_offers(
        ...
        to_date: Option<NaiveDate>,
        customer_contact_id: Option<i64>,
    ) -> impl std::future::Future<Output = Result<Page<OfferListItem>, ServerFnError>> + Send;
    fn get_offer(
        &self,
        id: i64,
    ) -> impl std::future::Future<Output = Result<Offer, ServerFnError>> + Send;
    fn save_offer(
        &self,
        offer: Offer,
    ) -> impl std::future::Future<Output = Result<Offer, ServerFnError>> + Send;
    ...
    fn get_offer_revisions(
        &self,
        offer_id: i64,
    ) -> impl std::future::Future<Output = Result<Vec<OfferRevision>, ServerFnError>> + Send;
    fn create_offer_revision(
        &self,
        offer_id: i64,
    ) -> impl std::future::Future<Output = Result<Offer, ServerFnError>> + Send;
    ...
    fn get_receipts(
        ...
        from_date: Option<NaiveDate>,
        to_date: Option<NaiveDate>,
    ) -> impl std::future::Future<Output = Result<Page<ReceiptListItem>, ServerFnError>> + Send;
    fn get_receipt(
        &self,
        id: i64,
    ) -> impl std::future::Future<Output = Result<Receipt, ServerFnError>> + Send;
    fn save_receipt(
        &self,
        receipt: Receipt,
    ) -> impl std::future::Future<Output = Result<Receipt, ServerFnError>> + Send;
    ...
    fn get_categories(
        &self,
    ) -> impl std::future::Future<Output = Result<Vec<ReceiptItemCategory>, ServerFnError>> + Send;
    fn add_receipt_payment(
        &self,
        receipt_id: i64,
        amount_cents: i64,
        date: NaiveDate,
    ) -> impl std::future::Future<Output = Result<(), ServerFnError>> + Send;
    fn delete_receipt_payment(
        &self,
        id: i64,
    ) -> impl std::future::Future<Output = Result<(), ServerFnError>> + Send;
